...
# --- CSS (사장님이 좋아하신 이미지 속 디자인 스타일 - 에러 방지 처리) ---
st.markdown("""
    <style>
    ...
    /* 가을판매가 파란 배지 */
    .fall-badge { background: #2563EB; color: white; padding: 5px 10px; border-radius: 6px; font-weight: 700; font-size: 14px; display: inline-block; }
    
    /* 변동액 배지 */
    .diff-badge { padding: 4px 8px; border-radius: 5px; font-weight: 700; font-size: 11px; min-width: 65px; text-align: center; display: inline-block; }
...
if search_q: disp_df = disp_df[disp_df['상품명'].str.contains(search_q, case=False) | disp_df['메모'].astype(str).str.contains(search_q, case=False)]
...
if selected_indices:
    ...
    with st.container():
        ...
        new_compu = ec2.number_input("컴퓨존기준가 수정", value=int(target['컴퓨존판매가']), step=1000)
