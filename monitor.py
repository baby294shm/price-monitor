...
st.markdown("""
    ...
    .price-area { display: flex; align-items: center; gap: 18px; flex-shrink: 0; }
    .price-box { text-align: center; width: 90px; }
    .label-txt { font-size: 10px; color: #94A3B8; margin-bottom: 4px; display: block; }
    
    /* 이미지 스타일 배지 */
    .fall-badge { background: #2563EB; color: white; padding: 6px 12px; border-radius: 6px; font-weight: 700; font-size: 14px; min-width: 80px; display: inline-block; }
    ...
    .open-btn {
        ...
        padding: 7px 15px; border-radius: 6px; font-size: 13px; font-weight: 600; transition: 0.2s;
    }
...
# --- 화면 구성 ---
st.markdown('<div style="font-size:24px; font-weight:700; color:#1E293B; margin-bottom:20px;">🌸 가을 가전 통합 관리자</div>', unsafe_allow_html=True)
...
with col_f1: s_name = st.text_input("🔍 상품명 또는 메모 검색", placeholder="검색어를 입력하세요...")
with col_f2: s_cat = st.selectbox("📂 카테고리 필터", CATEGORY_LIST)
...
with col_act2: refresh_btn = st.button("🔄 실시간 가격 갱신", use_container_width=True)
with col_act3: delete_btn = st.button("🗑️ 선택 상품 삭제", use_container_width=True)
...
for idx, row in df.iterrows():
    ...
    with c_row:
        ...
        # 가격비교 탭 최적화 (가을판매가 대신 변동 배지 강조)
        if sel_tab == "가격비교":
            price_html = f"""
                <div class="price-box"><span class="label-txt">기준가</span><div style="font-size:14px; color:#64748B;">{int(row['컴퓨존판매가']):,}</div></div>
                <div class="price-box"><span class="label-txt">실시간가</span><div style="font-size:14px; font-weight:700; color:#334155;">{int(row['실시간가']):,}</div></div>
        ...
        <div class="product-row">
            <div class="info-area">
                <div class="name-line">
                    <span style="font-size:10px; color:#3B82F6; background:#EFF6FF; padding:1px 6px; border-radius:4px; font-weight:700;">{row['카테고리']}</span>
                    <div class="name-text">{row['상품명']}</div>
                </div>
                <div class="memo-text">📝 {row['메모'] if row['메모'] != "-" else "비고 없음"}</div>
            </div>
...
if selected_indices:
    with st.container():
        ...
        new_my = e1.number_input("가을판매가 수정", value=int(target['가을판매가']), step=1000)
        new_cp = e2.number_input("기준가(컴퓨존) 수정", value=int(target['컴퓨존판매가']), step=1000)
        new_memo = e3.text_input("메모 수정", value=target['메모'])
        if e4.button("💾 변경사항 저장", use_container_width=True):
            st.session_state.df.at[edit_idx, '가을판매가'] = new_my
            st.session_state.df.at[edit_idx, '컴퓨존판매가'] = new_cp
