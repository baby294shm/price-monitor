...
expand_title = "📝 상품 정보 수정하기" if is_edit else "➕ 신규 상품 등록"
...
with st.expander(expand_title, expanded=is_edit):
    ...
    with c7:
        st.write("")
        if st.button("💾 저장 및 새로고침", use_container_width=True, type="primary"):
            ...
            st.switch_page(st.navigation([st.Page(lambda: None, title="refresh")]).pages[0]) # 안전한 갱신

    if is_edit and st.button("❌ 취소", use_container_width=True):
        st.session_state.edit_idx = None
        st.switch_page(st.navigation([st.Page(lambda: None, title="refresh")]).pages[0])
...
def render_list(df_in, tab_name):
    ...
    if d_df.empty:
        st.info(f"'{tab_name}' 탭에 내용이 없습니다.")
        return
    ...
    for idx, row in d_df.iterrows():
        with st.container(border=True):
            ...
            with col_price:
                ...
                if tab_name != "가격비교":
                    p1.markdown(f"<p class='price-label'>가을가</p><p class='price-value'>{int(row['가을판매가']):,}</p>", unsafe_allow_html=True)
                p2.markdown(f"<p class='price-label'>기준가</p><p class='price-value'>{int(row['컴퓨존판매가']):,}</p>", unsafe_allow_html=True)
                p3.markdown(f"<p class='price-label'>실시간</p><p class='price-value' style='color:#2563EB;'>{int(row['실시간가']):,}</p>", unsafe_allow_html=True)
                ...
                p4.markdown(f"<p class='price-label'>변동액</p><p class='price-value'>{diff_txt}</p>", unsafe_allow_html=True)
            with col_btn:
                if st.button("수정", key=f"btn_{tab_name}_{idx}"):
                    st.session_state.edit_idx = idx
                    st.rerun() # 수정 시에만 1회 호출
